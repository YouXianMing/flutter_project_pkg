...
@immutable
class ModStaggeredGridConfiguration {
  ///  Creates an object that holds the configuration of a staggered grid.
  const ModStaggeredGridConfiguration({
  ...
  final ModIndexedStaggeredTileBuilder staggeredTileBuilder;
  ...
  ModStaggeredTile? getStaggeredTile(int index) {
    ModStaggeredTile? tile;
  ...
  double _getStaggeredTileMainAxisExtent(ModStaggeredTile tile) {
    return tile.mainAxisExtent ?? (tile.mainAxisCellCount! * cellExtent) + (tile.mainAxisCellCount! - 1) * mainAxisSpacing;
  }

  /// Creates a staggered tile with the computed extent from the given tile.
  ModStaggeredTile? _normalizeStaggeredTile(ModStaggeredTile? staggeredTile) {
    if (staggeredTile == null) {
      ...
      if (staggeredTile.fitContent) {
        return ModStaggeredTile.fit(crossAxisCellCount);
      } else {
        return ModStaggeredTile.extent(crossAxisCellCount, _getStaggeredTileMainAxisExtent(staggeredTile));
      }
    }
  }
}
...
/// Describes the placement of a child in a [ModRenderSliverStaggeredGrid].
///
/// See also:
///
...
class ModSliverStaggeredGridGeometry {
  ...
  const ModSliverStaggeredGridGeometry({
  ...
  ModSliverStaggeredGridGeometry copyWith({
    double? scrollOffset,
    ...
    return ModSliverStaggeredGridGeometry(
...
class ModRenderSliverStaggeredGrid extends ModRenderSliverVariableSizeBoxAdaptor {
  ...
  ModRenderSliverStaggeredGrid({
    required ModRenderSliverVariableSizeBoxChildManager childManager,
    required ModSliverStaggeredGridDelegate gridDelegate,
  ...
  void setupParentData(RenderObject child) {
    if (child.parentData is! ModSliverVariableSizeBoxAdaptorParentData) {
      final data = ModSliverVariableSizeBoxAdaptorParentData();
  ...
  void performLayout() {
    ...
    final viewportOffsets = _pageSizeToViewportOffsets.putIfAbsent(pageSize, () => SplayTreeMap<int, _ViewportOffsets?>());
    ...
    for (var index = viewportOffset.firstChildIndex; mainAxisOffsets.any((o) => o <= targetEndScrollOffset); index++) {
      ModSliverStaggeredGridGeometry? geometry = getSliverStaggeredGeometry(index, configuration, mainAxisOffsets);
      if (geometry == null) {
      ...
      if (geometry.trailingScrollOffset >= viewportOffset!.trailingScrollOffset) {
        ...
        final nextViewportOffset = _ViewportOffsets(mainAxisOffsets, (nextPageIndex + 1) * pageSize, nextPageIndex, index);
  ...
  static ModSliverStaggeredGridGeometry? getSliverStaggeredGeometry(
    ...
    final geometry = ModSliverStaggeredGridGeometry(
      scrollOffset: scrollOffset,
      crossAxisOffset: crossAxisOffset,
...
abstract class ModSliverStaggeredGridDelegate {
  ...
  ModStaggeredGridConfiguration getConfiguration(SliverConstraints constraints);
  ...
  /// be different.
  bool shouldRelayout(ModSliverStaggeredGridDelegate oldDelegate) {
...
/// See also:
///
///  * [ModSliverStaggeredGridDelegate], which creates staggered grid layouts.
///  * StaggeredGridView, which can use this delegate to control the layout of its
///    tiles.
///  * SliverStaggeredGrid, which can use this delegate to control the layout of its
///    tiles.
///  * [ModRenderSliverStaggeredGrid], which can use this delegate to control the layout of
///    its tiles.
class ModSliverStaggeredGridDelegateWithFixedCrossAxisCount extends ModSliverStaggeredGridDelegate {
  ...
  const ModSliverStaggeredGridDelegateWithFixedCrossAxisCount({
    required this.crossAxisCount,
  ...
  ModStaggeredGridConfiguration getConfiguration(SliverConstraints constraints) {
    ...
    return ModStaggeredGridConfiguration(
  ...
  @override
  bool shouldRelayout(covariant ModSliverStaggeredGridDelegateWithFixedCrossAxisCount oldDelegate) {
...
///  * [ModRenderSliverStaggeredGrid], which can use this delegate to control the layout of
///    its tiles.
class ModSliverStaggeredGridDelegateWithMaxCrossAxisExtent extends ModSliverStaggeredGridDelegate {
  ...
  const ModSliverStaggeredGridDelegateWithMaxCrossAxisExtent({
  ...
  ModStaggeredGridConfiguration getConfiguration(SliverConstraints constraints) {
    assert(_debugAssertIsValid());
    final int crossAxisCount = ((constraints.crossAxisExtent + crossAxisSpacing) / (maxCrossAxisExtent + crossAxisSpacing)).ceil();
  ...
  @override
  bool shouldRelayout(covariant ModSliverStaggeredGridDelegateWithMaxCrossAxisExtent oldDelegate) {
