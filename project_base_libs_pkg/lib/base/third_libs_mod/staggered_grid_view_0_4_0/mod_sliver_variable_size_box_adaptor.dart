...
///
/// [ModRenderSliverVariableSizeBoxAdaptor] objects reify their children lazily to avoid
...
abstract class ModRenderSliverVariableSizeBoxChildManager {
  ...
  /// the [ModRenderSliverVariableSizeBoxAdaptor] object if they were not created during
  ...
  ///
  /// Called by [ModRenderSliverVariableSizeBoxAdaptor.collectGarbage], which itself is
  /// called from RenderSliverVariableSizeBoxAdaptor.performLayout.
  ///
  /// The index of the given child can be obtained using the
  /// [ModRenderSliverVariableSizeBoxAdaptor.indexOf] method, which reads it from the
  ...
  /// Called during [ModRenderSliverVariableSizeBoxAdaptor.adoptChild].
  ///
  ...
  /// children for the [ModRenderSliverVariableSizeBoxAdaptor] to fill the
  /// [SliverConstraints.remainingPaintExtent].
  ///
  /// Typically called unconditionally at the start of layout with false and
  /// then later called with true when the [ModRenderSliverVariableSizeBoxAdaptor]
  /// fails to create a child required to fill the
  /// [SliverConstraints.remainingPaintExtent].
  ///
  /// Useful for subclasses to determine whether newly added children could
  /// affect the visible contents of the [ModRenderSliverVariableSizeBoxAdaptor].
  ...
  /// modifications to the [ModRenderSliverVariableSizeBoxAdaptor]'s child list.
  ///
  /// This function always returns true.
  ///
  /// The manager is not required to track whether it is expecting modifications
  /// to the [ModRenderSliverVariableSizeBoxAdaptor]'s child list and can simply return
  /// true without making any assertions.
  bool debugAssertChildListLocked() => true;
}

/// Parent data structure used by [ModRenderSliverVariableSizeBoxAdaptor].
class ModSliverVariableSizeBoxAdaptorParentData extends SliverMultiBoxAdaptorParentData {
  ...
  /// [ModRenderSliverVariableSizeBoxAdaptor._keepAliveBucket].
...
///
/// [ModRenderSliverVariableSizeBoxAdaptor] is a base class for slivers that have multiple
...
abstract class ModRenderSliverVariableSizeBoxAdaptor extends RenderSliver
    with ModTileContainerRenderObjectMixin<RenderBox, ModSliverVariableSizeBoxAdaptorParentData>, RenderSliverWithKeepAliveMixin, RenderSliverHelpers {
  /// Creates a sliver with multiple box children.
  ///
  /// The [childManager] argument must not be null.
  ModRenderSliverVariableSizeBoxAdaptor({required ModRenderSliverVariableSizeBoxChildManager childManager}) : _childManager = childManager;

  @override
  void setupParentData(RenderObject child) {
    if (child.parentData is! ModSliverVariableSizeBoxAdaptorParentData) {
      child.parentData = ModSliverVariableSizeBoxAdaptorParentData();
    }
  }
  ...
  /// [ModRenderSliverVariableSizeBoxAdaptor] uses a [ModRenderSliverVariableSizeBoxChildManager] to
  /// create children during layout in order to fill the
  /// [SliverConstraints.remainingPaintExtent].
  @protected
  ModRenderSliverVariableSizeBoxChildManager get childManager => _childManager;
  final ModRenderSliverVariableSizeBoxChildManager _childManager;
  ...
  bool hitTestChildren(HitTestResult result, {required double mainAxisPosition, required double crossAxisPosition}) {
    for (final child in children) {
      if (hitTestBoxChild(BoxHitTestResult.wrap(result), child, mainAxisPosition: mainAxisPosition, crossAxisPosition: crossAxisPosition)) {
        return true;
      }
    }
    return false;
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(DiagnosticsNode.message(childCount > 0 ? 'currently live children: ${indices.join(',')}' : 'no children current live'));
  }
